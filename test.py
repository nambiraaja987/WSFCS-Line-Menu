...
BASE_DIR = "https://raw.githubusercontent.com/nambiraaja987/WSFCS-Line-Menu/refs/heads/main/"
        ...
        if day.get("date") == date_str:
            for item in day.get("menu_items", []):
                ...
                if food and food.get("name"):
                    name = food["name"]
                    if any(bad in name.upper() for bad in EXCLUDED_ITEMS): continue
                    items.append(name)
            break
    return items
        ...
        if day.get("date") == date_str:
            for item in day.get("menu_items", []):
                ...
                if is_header:
                    ...
                    if len(txt) > 2:
                        current_station = txt
                        if current_station not in categorized: categorized[current_station] = []
                    continue
                food = item.get("food")
                if food and food.get("name"):
                    name = food["name"]
...
with col1:
    if os.path.exists(WSFCS_LOGO): st.image(WSFCS_LOGO, width=150)
with col2:
    st.markdown("<h1 style='text-align: center;'>Line Menu Generator</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center;'>Select any date range below.</p>", unsafe_allow_html=True)
with col3:
    if os.path.exists(CHARTWELLS_LOGO): st.image(CHARTWELLS_LOGO, width=200)
